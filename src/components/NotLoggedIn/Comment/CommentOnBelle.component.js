...
import './CommentOnBelleStyles.css';
...
export default class CommentOnBelle extends Component {
    constructor(props) {
        super(props)
    ...
    componentDidMount() {
        ...
        const formInfo = {
            belleId: this.props.belleId,
            sessionId: localStorage.getItem('e'),
        }

        axios.post('http://localhost:5000/get_belle_comments', formInfo)
            .then(response => {
    ...
    handleChangeFile = e => {
        ...
        let belleCommentsFiles_temp = this.state.belleCommentsFiles;
        belleCommentsFiles_temp[Object.keys(belleCommentsFiles_temp).length] = file;

        this.setState({
            belleCommentsFiles: belleCommentsFiles_temp,
        }, () => {
            console.log(this.state.belleCommentsFiles)
        })
    }
    ...
    submitComment(e) {
        ...
        Object.keys(this.state.belleCommentsFiles).map(index => {
            fileData.append("files", this.state.belleCommentsFiles[index]);
        })

        fileData.append("sessionId", localStorage.getItem('e'));
        fileData.append("content", this.state.comment);
        fileData.append("belleId", this.props.belleId);

        axios.post('http://localhost:5000/comment_on_belle', fileData, {
            headers: {
